...
(`source`/`text`/`language`/`default_speaker`/`phonemes` are set with
`setdefault`, so the first generation wins).

`source` records who created the reference and settles on first write:

| Value | Meaning |
| --- | --- |
| `"seed"` (default) | Pre-generated by `pregenerate_references.py` / the `generate-reference` CLI — **never aged out** |
| `"user"` | Created via the API (`POST /references`, `POST /evaluate/quick`) — eligible for retention cleanup |

Existing references that predate the field have no `source` and are treated as
`"seed"`, so the bundled references are protected with no migration.

## Retention

User-generated references (`source="user"`) accumulate as visitors use the
demo. A background task (`cleanup_old_references`, scheduled from the app
lifespan) deletes those whose `updated_at` is older than the retention window.
Seed references are skipped outright, so the hundreds of bundled references are
never touched.

| Env var | Default | Effect |
| --- | --- | --- |
| `REFERENCES_RETENTION_HOURS` | `1` | User references older than this are deleted. `0` = keep forever |
| `REFERENCES_CLEANUP_INTERVAL_HOURS` | `1` | How often the background sweep runs |
